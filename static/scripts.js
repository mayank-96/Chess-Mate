...
var updateStatus = function(data3) {
  var status = '';
  ...
  // game still on
    ...
    status = moveColor + ' to move' + data3;
...
var randomResponse = function() {
    fen = game.fen()
    $.get($SCRIPT_ROOT + "/move/" + fen, function(data1,data2) {
        game.move(data1, {sloppy: true});
        game.move(data2, {sloppy: true});
        updateStatus(data2);
        document.getElementById("smove").innerHTML = data2;
        board.position(game.fen());
        updateStatus();
    })
}
...
var getResponseMove = function() {
    ...
    fen = game.fen()
    $.get($SCRIPT_ROOT + "/move/" + depth + "/" + fen, function(data1,data2) {
        game.move(data1, {sloppy: true});
        game.move(data2, {sloppy: true});
        updateStatus(data2);
        document.getElementById("smove").innerHTML = data2;
